...
static int mov_read_mhac(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    uint8_t configurationVersion = avio_r8(pb);
    if(configurationVersion != 0x01) {
        av_log(c->fc, AV_LOG_ERROR, "unsupported mhac version");
        return AVERROR_INVALIDDATA;
    }
    par->profile = avio_r8(pb);

    /* use codec-specific field to store initial referenceChannelLayout,
     * because the native AVCodecParameters::channel_layout might be overridden
     * by ffmpeg */
    par->level = avio_r8(pb);
    int config_size = avio_rb16(pb);
