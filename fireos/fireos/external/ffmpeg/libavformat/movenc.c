...
{
    avio_wb32(pb, track->vos_len + 8 + 5);
    ffio_wfourcc(pb, "mhaC");

    /* See ISO/IEC 23008-3:2019 20.4.2 */
    avio_w8(pb, 0x01); /* version */
    avio_w8(pb, track->par->profile); /* profile */
    avio_w8(pb, track->par->level); /* reference channel layout */
    avio_wb16(pb, (uint16_t)track->vos_len); /* config size */
    avio_write(pb, track->vos_data, track->vos_len); /* configuration */
    return 8 + 5 + track->vos_len;
}
    ...
    if (version == 2) {
        ...
        if (track->mode == MODE_MOV) {
            ...
            if (track->par->codec_id == AV_CODEC_ID_FLAC ||
                ...
                avio_wb16(pb, track->par->channels);
            } else if (track->par->codec_id == AV_CODEC_ID_XHEAAC) {
                /* Channels count is inapplicable for MHA decoder
                 * and should be set to 0.
                 * See ISO/IEC 23008-3-2019 20.5.3 for details. */
                avio_wb16(pb, 0);
            } else {
                avio_wb16(pb, 2);
            }
