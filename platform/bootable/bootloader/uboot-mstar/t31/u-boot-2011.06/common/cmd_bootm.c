...
//Mstar Start
    ...
    if (isBootToRecovery() || ENABLE_MODULE_TEE == 0)
	{
		...
		if(!is_quiescent_mode()) {
            ...
            if (!strcmp(amzn_target_device_name(), "ABC")) {
                printf("Silent OTA, do not thing\n");
            } else {
                ...
                {
                    MsDrv_PNL_BackLight_On();
                    printf("Silent OTA, turn on the backlight as the VCC for Ldm led device APE5030, but disable all led channel.\n");
                }
                else
                {
                    printf("Silent OTA, do not turn on backlight\n");
                    MApi_PNL_SetBackLight(0);
                }
#else
                printf("Silent OTA, do not turn on backlight\n");
                MApi_PNL_SetBackLight(0);
...
{
    char *cmdline_arg_start = NULL;
    char *arg_start = NULL;
    char *arg_end = NULL;
    char BUILDVARIANT_ARG[]="buildvariant=";
    char VERITYKEYID_ARG[]="veritykeyid=";
	...
	*newbootargs = '\0';
	...
	if (bootargs) {
		...
		if (*hdr->cmdline) {
			//remove duplicated buildvariant in bootargs and use cmdline arg
			cmdline_arg_start = strstr(hdr->cmdline, BUILDVARIANT_ARG);
			arg_start = strstr(newbootargs, BUILDVARIANT_ARG);
			if (cmdline_arg_start != NULL && arg_start != NULL) {
				arg_end = strstr(arg_start, " ");
				if (arg_end == NULL) {
					*arg_start = '\0';
				} else {
					strcpy(arg_start, arg_end);
				}
			}

			//remove duplicated veritykeyid in bootargs and use cmdline arg
			cmdline_arg_start = strstr(hdr->cmdline, VERITYKEYID_ARG);
			arg_start = strstr(newbootargs, VERITYKEYID_ARG);
			if (cmdline_arg_start != NULL && arg_start != NULL) {
				arg_end = strstr(arg_start, " ");
				if (arg_end == NULL) {
					*arg_start = '\0';
	...
	if (*hdr->cmdline) {
		...
		char* hdr_cmdline = strdup(hdr->cmdline);
		if (hdr_cmdline && strlen(hdr_cmdline)) {
			char* removed_sec_cmdline = str_replace(hdr_cmdline, "androidboot.unlocked_kernel=true", " ");

			if (removed_sec_cmdline) {
				strcpy(hdr_cmdline,removed_sec_cmdline);
				free(removed_sec_cmdline);
			}
			removed_sec_cmdline = str_replace(hdr_cmdline, "androidboot.veritymode=disabled", " ");
			if (removed_sec_cmdline) {
				strcpy(hdr_cmdline,removed_sec_cmdline);
				free(removed_sec_cmdline);
			}
			removed_sec_cmdline = str_replace(hdr_cmdline, "androidboot.prod=0", " ");
			if (removed_sec_cmdline) {
				strcpy(hdr_cmdline,removed_sec_cmdline);
				free(removed_sec_cmdline);
			}
			removed_sec_cmdline = str_replace(hdr_cmdline, "androidboot.selinux=permissive", " ");
			if (removed_sec_cmdline) {
				strcpy(hdr_cmdline,removed_sec_cmdline);
				free(removed_sec_cmdline);
			}
			removed_sec_cmdline = str_replace(hdr_cmdline, "loglevel=7", " ");
			if (removed_sec_cmdline) {
				strcpy(hdr_cmdline,removed_sec_cmdline);
				free(removed_sec_cmdline);
			}

			/*to avoid xxx=1 xxx=0 case, if exists, will remove firstly, and append at the end of line*/
			if (strlen(newbootargs)) {
				char* new_newbootargs = str_replace(newbootargs, hdr_cmdline, "");
				if (new_newbootargs) {
					strcpy(newbootargs, new_newbootargs);
					free(new_newbootargs);
				}
			}
			strcat(newbootargs, hdr_cmdline);
            free(hdr_cmdline);
		}
	}
