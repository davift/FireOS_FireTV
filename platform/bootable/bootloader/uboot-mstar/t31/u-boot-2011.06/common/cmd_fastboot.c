...
static void fastboot_publish(char *name, char *value, void (*handle)(const char,const char,int))
{
}
#else
...
static void get_download_addr()
{
    ...
    download_addr = 0x29800000; // Luke modify
...
void fb_init(unsigned short net)
{
    ...
    fastboot_publish("slot-retry-count:", "",do_getvar_slotRetryCount);
        ...
        #ifdef UFBL_FEATURE_IDME
        {
            ...
            /* following if condition is only for ANNA as AMMO */
            /* is enabled in the middle of ANNA development */
            if (!strcmp(amzn_target_device_name(), "anna") && !(strcmp(ammo_pv, "")))
                sprintf(ammo_pv, "%s", "anna-am");
            fastboot_publish("product-variant", ammo_pv, NULL);
            strcpy(fb_status.message, ammo_pv);
        }
        #else
            fastboot_publish("product-variant", kProduct, NULL);
        #endif
    #endif
}
...
static void cmd_oem_bootmode(char *arg, unsigned buffer_len)
{
    ...
    if(ret) {
        snprintf(response, sizeof(response), "Could not set bootmode, error=%d", ret);
...
static void cmd_getvar(char *buffer, unsigned buffer_len)
{
    ...
    {
#ifdef TARGET_AMMO_SUPPORT
        if (!strcmp_l1("product-variant", buffer))
        {
#ifdef UFBL_FEATURE_IDME
            char ammo_pv[PROD_VAR_SIZE+1] = {0,};
            idme_get_oem_data_field("ammo_var=", ammo_pv, PROD_VAR_SIZE);
...
#define HEX_ADDR_STR_MAX_LEN    18

// return 0 if the hex addr string is invalid; return 1 if it is valid
static int validate_hex_addr_string(char *name) {
    int i = 0;
    int len = strlen(name);

    if (len > HEX_ADDR_STR_MAX_LEN) {
        return 0 ;
    }

    // define appropriate character set for hex addr string
    for (i = 0; i < len; ++i) {
        if (('a' <= name[i] && name[i] <= 'f')
              || ('A' <= name[i] && name[i] <= 'F')
              || ('0' <= name[i] && name[i] <= '9'))
            continue;

        if ((i == 1) && (name[i-1] == '0') && (name[i] == 'x'))
            continue;
...
static void cmd_flash(char *buffer, unsigned buffer_len)
{
    ...
    // For pre-burn image
    if (!strcmp_l1(STR_BURN, buffer))
    {
        // ex: fastboot -s udp:192.168.1.101 flash BURNAF000 ./images/ext4/x.81
        // partition name is BURNAF000. The string after keyword BURN must be a hex addr.
        offset = strlen(STR_BURN);
        if (!validate_hex_addr_string(buffer+offset)) {
            fb_set_resp(kStatusError, "invalid arguments");
            printf("ERROR: Invalid hex addr string %s in partition name %s\n", buffer+offset, partition_name);
            return;
        }

        // exmpl: mmc write 0x20200000 AF000 0x3200000
        snprintf(tmpCmd, sizeof(tmpCmd), "mmc write 0x%08x %s 0x%08x",
               download_addr, buffer+offset, fb_download.received);
...
static void cmd_usb_download(char *buffer, unsigned buffer_len)
{
    fb_status.status_code = kStatusData;
    do_download(NULL, buffer, buffer_len);
}
