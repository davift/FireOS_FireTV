...
__u16 get_emmc_partition_number(block_dev_desc_t *dev_desc)
{
    if (check_partition_table_version(dev_desc, &g_ddesc) < 0)
    {
            return 0;
    ...
    __u16 partnum = 1;
    for (partnum = 1; partnum <= g_emmc_reserved_for_map; partnum++)
    {
        if (g_mpart[partnum-1].signature != EMMC_PARTITION_MAGIC)
            break;
    }
