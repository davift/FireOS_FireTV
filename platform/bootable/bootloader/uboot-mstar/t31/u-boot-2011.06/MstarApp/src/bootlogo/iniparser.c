...
int Profile_TranslateU8Array(const char *pszIniString, const unsigned int u16OutDataLen, unsigned char *pu8OutDataVal)
{
    ...
    unsigned char szToken[10] = { 0 };
    unsigned int u16ReadIdx = 0;
    unsigned int u16WriteIdx = 0;
    unsigned int u16CheckHex = 0;

    unsigned int u16OutDataIdx = 0;
    unsigned char u8Carry = 0;
    unsigned long u32TempValue = 0;
    unsigned long u32TempOutDataVal = 0;

    memset(pu8OutDataVal, 0, u16OutDataLen);
    u16Len = strlen((const char *)(pszIniString));
    //printf("u16Len %d\n", u16Len);
    if(0 == u16Len)
    {
        return 0;
    ...
    //skip space
    //
    while((pszIniString[u16ReadIdx] == ' ') && (u16ReadIdx < u16Len))
    {
        u16ReadIdx++;
    }
    ...
    //
    if(pszIniString[u16ReadIdx] == '{')
    {
        u16ReadIdx++;
    }
    else
    {
        printf("ERROR: Wrong format in ini file :: 1 \n");
        return 0;
    }
    ...
    {
        //check 0~9
        if(!((pszIniString[u16ReadIdx] >= '0') && (pszIniString[u16ReadIdx] <= '9')))
        {
            printf("ERROR: Wrong format in ini file ::  2 - %c  \n", pszIniString[u16ReadIdx]);
            return 0;
        }

        //check 0~9, but not ',' or '}'
        u16WriteIdx = 0;
        while(((pszIniString[u16ReadIdx] >= '0') && (pszIniString[u16ReadIdx] <= '9')) || ((pszIniString[u16ReadIdx] != ',') && (pszIniString[u16ReadIdx] != '}')))
        {
            szToken[u16WriteIdx] = pszIniString[u16ReadIdx];
            u16WriteIdx++;
            u16ReadIdx++;

            u16CheckHex = 0;
            while((pszIniString[u16ReadIdx] == 'x') || ((pszIniString[u16ReadIdx] >= '0') && (pszIniString[u16ReadIdx] <= '9')) || ((pszIniString[u16ReadIdx] >= 'a') && (pszIniString[u16ReadIdx] <= 'f')) || (pszIniString[u16ReadIdx] == 'X') || ((pszIniString[u16ReadIdx] >= 'A') && (pszIniString[u16ReadIdx] <= 'F')))
            {
                szToken[u16WriteIdx] = pszIniString[u16ReadIdx];
                u16WriteIdx++;
                u16ReadIdx++;
                u16CheckHex++;
                if(((pszIniString[u16ReadIdx] == 'x') || (pszIniString[u16ReadIdx] == 'X')) && (u16CheckHex != 1))
                {
                    printf("ERROR: Wrong format in ini file ::  3 \n");
                    return 0;
                }
            }

            //skip space
            while((pszIniString[u16ReadIdx] == ' '))
            {
                u16ReadIdx++;
            }
        }
        ...
        if(pszIniString[u16ReadIdx] == ',')
        {
            szToken[u16WriteIdx] = '\0';
            u16ReadIdx++;
        }
        else if(pszIniString[u16ReadIdx] == '}')
        {
            szToken[u16WriteIdx] = '\0';
        }
        else
        {
            printf("ERROR: Wrong format in ini file :: 4 - %c  \n", pszIniString[u16ReadIdx]);
            return 0;
        }
        ...
        //
        u16WriteIdx = 0;
        if((szToken[u16WriteIdx + 1] == 'x') || (szToken[u16WriteIdx + 1] == 'X'))
        {
            u8Carry = 16;
            u16WriteIdx = u16WriteIdx + 2;
        }
        else
        {
            u8Carry = 10;
        ...
        u32TempOutDataVal = 0;
        while(szToken[u16WriteIdx] != '\0')
        {
            if((szToken[u16WriteIdx] >= '0') && (szToken[u16WriteIdx] <= '9'))
            {
                u32TempValue = (szToken[u16WriteIdx] - '0');
            }
            else if((szToken[u16WriteIdx]>='A') && (szToken[u16WriteIdx]<='F'))
            {
                u32TempValue = (szToken[u16WriteIdx] - 'A' + 10);
            }
            else if((szToken[u16WriteIdx]>='a') && (szToken[u16WriteIdx]<='f'))
            {
                u32TempValue = (szToken[u16WriteIdx] -'a' + 10);
            ...
            u32TempOutDataVal = u32TempValue + u32TempOutDataVal * u8Carry;
            u16WriteIdx++;
        }

        //check overflow
        if(u32TempOutDataVal > 0xFF)
        {
            printf("ERROR: Wrong format in ini file :: 5\n");
            return 0;
        }

        pu8OutDataVal[u16OutDataIdx] = u32TempOutDataVal;
        u16OutDataIdx++;
    ...
    if(u16ReadIdx >= u16Len)
    {
        printf("ERROR: Wrong format in ini file :: 6\n");
        return 0;
    }
