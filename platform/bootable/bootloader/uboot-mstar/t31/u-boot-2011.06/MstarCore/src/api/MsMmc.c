...
#if (ENABLE_MODULE_MMC == 1)
    snprintf(cmd, sizeof(cmd)-1, "mmc write.p 0x%lx misc 0x%x", (long)&p_msg, (u32)sizeof(struct bootloader_message));
#else
    //nand is not supportted now
...
static int cmd_crcpart(cmd_tbl_t *cmdtp_t, int flag_t, int argc_t, char * const argv_t[])
{
     u32 blkChunk = 2095;
     u32 buf_size = blkChunk*512;
     u32 blkStart = 0, blkCount = 0, traceCount = 0;
    ...
    for (i=0; i<EMMC_RESERVED_FOR_MAP_V2; i++)
    {
        ...
        if(!strncmp(g_mpart[i].name, argv_t[2], strlen(g_mpart[i].name))) {
            printf("Find partition:%s to calculate CRC\n", g_mpart[i].name);
            break;
        }
    }
                    //printf("\nread eMMC device with %d blocks every time\n", blkChunk);
      blkStart = g_mpart[i].start_block;
      totalBlock = g_mpart[i].block_count;

      printf("start_block:%d, block_count:%d\n", blkStart, totalBlock);
     ...
     free(addr);
     addr = NULL;
     printf("\nMMC read: dev # %d, partition:%s, block # %d, count %u success\n",
                                curr_device, g_mpart[i].name, g_mpart[i].start_block, (u32)(g_mpart[i].block_count));
         ...
         cur_num = cur*512/8;
            ...
            for(index=0; index< cur_num; ++index)
            {
                if((glo_now_index<glo_pre_start) || (glo_now_index>glo_pre_end))
                {
                    ...
                    {
                           printf("static reliable write test fail, startblk %llu index %llu, l 0x%llx r 0x%llx\n", start, index,
                            p[index], (start + (index*8) / 512));
                  printf("static reliable write test fail, the start addr is 0x%llx\n",(start*512+index*8));
                        sprintf(buf, "%d", 0);
