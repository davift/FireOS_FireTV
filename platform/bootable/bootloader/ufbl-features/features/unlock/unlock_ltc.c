	...
	const unsigned char *key, unsigned int key_len)
{
	...
	int hash_memory_ret = hash_memory(sha256_idx, data, data_len, digest, &digest_len);
	if (hash_memory_ret != CRYPT_OK) {
		dprintf("Failed hash memory, ret=%d\n", hash_memory_ret);
		/* Enter infinity loop when hash_memory returns error */
		while (1);
	}
