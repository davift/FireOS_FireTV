...
ASFParser::asf_parse_read_object(asfint_object_t *obj, uint8_t *data)
{
    ...
    {
        ALOGV("Object: %x-%x-%x-%02x%02x%02x%02x%02x%02x%02x%02x, %ld bytes\n",
        ...
        ALOGV("obj->type=%d,size=%lld,datalen=%lld\n",
...
ASFParser::asf_parse_header()
{
     ...
     * the subobject count field and two reserved fields
     Check if it has at least 30 bytes => GUID(16) Size(8) NumSubObj(4) Res1(1) Res2(1))
     */
    memset(hdata, 0, 30);
    ...
    if (header->subobjects > 0)
    {
        ...
        for (i=0; i<header->subobjects; i++)
        {
            ...
            if (!tmp) {
                ALOGE("Err 1:asf_parse_header ASF_ERROR_OUTOFMEM");
                return ASF_ERROR_OUTOFMEM;
            }
            ...
            if (current->type == GUID_HEADER_EXTENSION && !header->ext)
            {
                ...
                if(!tmp)
                {
                    ALOGE("Err:asf_parse_header, realloc fail!");
                    return ASF_ERROR_OUTOFMEM;
                }
                current = (asfint_object_t *)tmp;
...
ASFParser::asf_parse_data()
{
    ...
    if ((data->total_data_packets) && (file->data_packets_count) &&
...
ASFParser::asf_parse_index_index()
{
    ...
    //parser 1ht blocks
    tmp = ASFByteIO::asf_byteio_read(block_data,12, iostream);
    ...
    ALOGI("---Index Blocks,:index_block_position=%d----\n",(int)index_block_position);



    entry_data_size = index_entry_count * 4;//Block Positions 8 + Offsets 4
    entry_data = (uint8_t*)calloc(1,entry_data_size * sizeof(uint8_t));
     ...
     {
         index->index_block->index_entry[i].offset = ASFByteIO::asf_byteio_getDWLE(entry_data+i*4);
     }
...
int
ASFParser::asf_do_parse_index(int64_t next_index_position)
{
    asf_object_index_t *index = NULL;
    asf_iostream_t *iostream = NULL;
    ...
    if (tmp < 0) {
            ALOGE("asf_do_parse_index:error1 tmp=%d",tmp);
            return tmp;//ASF_ERROR_EOF or ASF_ERROR_IO;
    }
    ALOGV("asf_do_parse_index:tmp1=%d",tmp);
    // allocate the index object
    index = (asf_object_index_t*)calloc(1,sizeof(asf_object_index_t));
    if (!index) {
            ALOGE("asf_do_parse_index:ASF_ERROR_OUTOFMEM 0\n");
            return ASF_ERROR_OUTOFMEM;
    }
    asf_parse_read_object((asfint_object_t *) index, idata);
...
int
ASFParser::asf_parse_index()
{
    int tmp;
    int64_t seek_position;

    ALOGI("parse index table");

    if (mHasParsedIndexTab) {
        return ASF_PARSE_SUCCESS;
    }

    if (file->flags & ASF_FLAG_SEEKABLE && file->iostream.seek)
    {
        ALOGV("data_position=%lld, data_size=%lld\n", file->data_position, file->data->size);

        file->index_position = file->data_position + file->data->size;

        //go to index pos to parse idx
        seek_position = file->iostream.seek(file->iostream.source, file->index_position);
        ...
        if (seek_position >= 0) {
            ...
            {
                ALOGV("asf_parse_index next_index_position=%lld\n", next_index_position);

                tmp = asf_do_parse_index(next_index_position);
                if (tmp < 0)
                {
                    ALOGV("asf_parse_index,tmp=%d\n", tmp);
                    break;
                }
                tmp_position_inc = tmp_position_inc + tmp;
                next_index_position = file->index_position + tmp_position_inc;

                //parse the next index obj
                seek_position = file->iostream.seek(file->iostream.source, next_index_position);
                if (seek_position < 0) {
                    ALOGE("asf_parse_index: iostream.seek fail %lld\n", seek_position);
                }
            }

            ALOGV("asf_parse_index: next_index_position=%lld, file_size=%lld\n",next_index_position,file->file_size);

            if (!file->simple_index && !file->index)
            {
               ALOGE("asf_parse_index: Couldn't find an index object\n");
               file->index_position = 0;
            }

            //after parse index, will reset the read position to the data->packets_position
            seek_position = file->iostream.seek(file->iostream.source, file->data->packets_position);
