...
//#include <string.h>
#undef LOG_TAG
#define LOG_TAG "ASF Header"

#include "asfparser.h"

#include <media/stagefright/foundation/AString.h>

#define PICPREVIEW "StereoscopicPreview"
#define ISO_639_1_LANG_NUM    201

typedef struct _ASF_LANG_639_T
{
    char two_cc[3];
    char three_cc[4];
}ASF_LANG_639_T;

static ASF_LANG_639_T lang_639_array[ISO_639_1_LANG_NUM]=
{
    {"aa", "aar"},
    {"ab", "abk"},
    {"af", "afr"},
...
void asf_lang_639_1_to_639_2(char* iso_639_1, char* iso_639_2)
{
    uint16_t      ui2_idx;

    if((NULL == iso_639_1) || (NULL == iso_639_2)) {
        return;
    }

    memset(iso_639_2, 0, 4);
    for(ui2_idx = 0; ui2_idx < ISO_639_1_LANG_NUM; ++ui2_idx) {
        if (0 == strncmp((char *)iso_639_1, (char *)lang_639_array[ui2_idx].two_cc, 2)) {
            strncpy((char *)iso_639_2, (char *)lang_639_array[ui2_idx].three_cc, 3);
            break;
        }
    }
    return;
                                   ...
                                   uint32_t objsize)
{
        ...
        case GUID_STREAM_TYPE_EXTENDED_AUDIO:
        {
                ...
                if (ASFByteIO::asf_byteio_getWLE(data + 16) > datalen - 16)
                {//WAVEFORMATEX start from 16
                       return ASF_ERROR_INVALID_LENGTH;
                }
        ...
        case GUID_STREAM_TYPE_VIDEO:
        {
                ...
                bmih->biPlanes = (uint16_t)(ASFByteIO::asf_byteio_getDWLE(data + 12));
                bmih->biBitCount = (uint16_t)(ASFByteIO::asf_byteio_getDWLE(data + 14));
                                                  ...
                                                  uint32_t objsize)
{
        ...
        cur_des_left_len = (uint16_t)objsize;
    ...
    //                extended_content_description->extended_content_num,cur_des_left_len,cur_des_max_addr);
        ...
        for(idx=0;idx<extended_content_description->extended_content_num;idx++)
        {
            ...
            {
                ALOGE("Err:cur_des_name_char = NULL");
                return -1;
            }
                                                  ...
                                                  uint32_t objsize)
{
       ...
       ALOGV("---asf_parse_index_parameters---\n");
       ALOGV("---index_entry_time_interval=%d---\n",index_parameters->index_entry_time_interval);
       ALOGV("---index_specifiers_count=%d---\n",index_parameters->index_specifiers_count);
       ...
       if(index_parameters->index_specifiers_entry != NULL)
       {
        ...
        if (header->first)
        {
            ...
            while (current)
            {
                ...
                case GUID_FILE_PROPERTIES:
                {
                    ...
                    file->real_duration = (int64_t)(file->play_duration/10000 - file->preroll) > 0 ? (file->play_duration/10000 - file->preroll) : 0;
                    ...
                    {
                        file->file_size = 0;
                        file->data_packets_count = 0;
                        file->play_duration = 0;
                        file->send_duration = 0;
                        file->real_duration = 0;
                    }
					#endif
        ...
        if (header->ext)
        {
             ...
             while (current)
             {
                  ...
                  case GUID_LANGUAGE_LIST:
                  {
                      ...
                      if (size < 26)
                        return ASF_ERROR_OBJECT_SIZE;
                      uint64_t ObjSize = size;
                      uint16_t LangIDCount = ASFByteIO::asf_byteio_getWLE(current->data);
                      uint16_t i,j,k, LangIDLength, position = 2;
                      for(i=0; i<LangIDCount; i++) {
                        k = 0;
                        if((position > ObjSize) || (i > 12))
                            break;
                        LangIDLength = current->data[position];
                        position++;
                        if((!LangIDLength) || (LangIDLength > 32))
                            LangIDLength = 6;
                        for(j=0; j < LangIDLength; j++) {
                            if(current->data[position+j] != 0x00) {
                                lang[k] = current->data[position+j];
                                k++;
                            }
                        }
                        position += LangIDLength;
                        asf_lang_639_1_to_639_2(lang, language[i]);
                        ALOGD("GUID_LANGUAGE_LIST: LangIDCount %02x LangIDLength %02x lang %s language %s", \
                        LangIDCount, LangIDLength, lang, language[i]);
                      }
                  }
                  break;
                  ...
                  case GUID_EXTENDED_STREAM_PROPERTIES:
                  {
                      ...
                      ret = asf_parse_header_extended_stream_properties(stream,
                                                                        current->data,
                                                                        size);
                      memcpy(stream->extended->language, language[stream->extended->lang_idx], 4);
...
ASFParser::asf_header_metadata(asf_object_header_t *header)
{
    ...
    // init the asf_metadata_t
    memset(ret,0,sizeof(asf_metadata_t));
    ...
    if (current)
    {
        ...
        for (i=0; i<5; i++)
        {
            ...
            case 0:
                    ...
                    snprintf(ret->content[i].key,6,"%s","Title");
            ...
            case 1:
                    ...
                    snprintf(ret->content[i].key,7, "%s","Author");
                    ret->content[i].value = str;
                    ret->content[i].size = 7;
                    break;
            case 2:
                     ret->content[i].key = (char*)calloc(10, sizeof(char));
                    snprintf(ret->content[i].key,10, "%s","Copyright");
                    ret->content[i].value = str;
                    ret->content[i].size = 10;
                    break;
            case 3:
                     ret->content[i].key = (char*)calloc(12, sizeof(char));
                    snprintf(ret->content[i].key,12, "%s","Description");
                    ret->content[i].value = str;
                    ret->content[i].size = 12;
                    break;
            case 4:
                     ret->content[i].key = (char*)calloc(7, sizeof(char));
                    snprintf(ret->content[i].key,7, "%s","Rating");
                    ret->content[i].value = str;
                    ret->content[i].size = 7;
                    break;
            default:
    ...
    if (current)
    {
            ...
            if (!ret->extended)
            {
                ...
                {
                    if (ret->content[i].key)
                    free(ret->content[i].key);

                    if (ret->content[i].value)
                    free(ret->content[i].value);
                }
            ...
            for (i=0; i<ret->extended_count; i++)
            {
                ...
                if (!(ret->extended[i].size) || ret->extended[i].size>current->datalen || !strncmp(ret->extended[i].key, PICPREVIEW, 19))
                {
                     ALOGE("error in parse %d extend strlen=%d",i,length);
          ...
          for (i=0; i<ret->extended_count; i++)
         {
            ...
            for (i=0; i<ret->metadata_count; i++)
            {
                ...
                case 1:
                        ...
                        ret->metadata[i].value[j+1] = '\0';
                        break;
                case 2:
                        /* type of the value is a boolean */
                        ret->metadata[i].value = (char*)calloc(6, sizeof(char));
                        snprintf(ret->metadata[i].value,6, "%s",
                                *current->data ? "true" : "false");
                        break;
                case 3:
                        /* type of the value is a signed 32-bit integer */
                        ret->metadata[i].value = (char*)calloc(11, sizeof(char));
                        snprintf(ret->metadata[i].value,11, "%u",
                                ASFByteIO::asf_byteio_getDWLE(current->data + position));
                        break;
                case 4:
                        /* FIXME: This doesn't print whole 64-bit integer */
                        ret->metadata[i].value = (char*)calloc(21, sizeof(char));
                        snprintf(ret->metadata[i].value,21, "%u",
                                (uint32_t) ASFByteIO::asf_byteio_getQWLE(current->data + position));
                        break;
                case 5:
                        /* type of the value is a signed 16-bit integer */
                        ret->metadata[i].value = (char*)calloc(6, sizeof(char));
                        snprintf(ret->metadata[i].value,6, "%u",
                                ASFByteIO::asf_byteio_getWLE(current->data + position));
                        break;
                default:
                        /* Unknown value type... */
    ...
    if (current)
    {
        ...
        for (i=0; i<ret->metadatalib_count; i++)
        {
            ...
            case 2:
                    ...
                    snprintf(ret->metadatalib[i].value,6, "%s",
                            *current->data ? "true" : "false");
                    break;
            case 3:
                    /* type of the value is a signed 32-bit integer */
                    ret->metadatalib[i].value = (char*)calloc(11, sizeof(char));
                    snprintf(ret->metadatalib[i].value,11, "%u",
                            ASFByteIO::asf_byteio_getDWLE(current->data + position));
                    break;
            case 4:
                    /* FIXME: This doesn't print whole 64-bit integer */
                    ret->metadatalib[i].value = (char*)calloc(21, sizeof(char));
                    snprintf(ret->metadatalib[i].value,21, "%u",
                            (uint32_t) ASFByteIO::asf_byteio_getQWLE(current->data + position));
                    break;
            case 5:
                    /* type of the value is a signed 16-bit integer */
                    ret->metadatalib[i].value = (char*)calloc(6, sizeof(char));
                    snprintf(ret->metadatalib[i].value,6, "%u",
                            ASFByteIO::asf_byteio_getWLE(current->data + position));
                    break;
            default:
                    /* Unknown value type... */
...
ASFParser::asf_header_free_metadata(asf_metadata_t *metadata)
{
    ...
    if(metadata->metadatalib_count > 0) {
