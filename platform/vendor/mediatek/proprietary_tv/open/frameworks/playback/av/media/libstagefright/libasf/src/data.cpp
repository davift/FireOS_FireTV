                       ...
                       uint32_t datalen)
{
        ...
        if (packet->payload_count == 0) {
                ALOGE("no payloads in the packet");
...
int ASFParser::asf_data_get_packet(asf_packet_t *packet)
{
        ...
        uint32_t read = 0;
        uint8_t packet_len_flags, packet_property, payload_length_type;
        void *tmpptr;
        int tmp;
        ...
        if (tmp & 0x80) {
                ...
                packet_len_flags = packet->data[read++];
        } else {
                packet->ec_length = 0;
                /*if error flag not set, data start with Payload*/
                packet_len_flags = (uint8_t)tmp;
        }
        ...
        tmp = asf_data_read_payloads(packet, file->preroll, packet_len_flags & 0x01,
