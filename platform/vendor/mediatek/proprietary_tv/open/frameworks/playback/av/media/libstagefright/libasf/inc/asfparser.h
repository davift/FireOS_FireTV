...
public:
        ...
        /**
          ...
          * @returns error type.
          */
        //ASFErrorType    ParseAsfFile();  not used now
        ...
        int asf_get_stream_packet(asf_packet_t *packet, uint32_t aStreamID, bool isForward);

        /* get next packet from the stream to the specified packet structure */
        int asf_get_packet(asf_packet_t *packet);

        int asf_get_track_pts(uint32_t track, int64_t* pi8_pts, bool isForward);

        uint64_t asf_seek_file(int64_t msec);

        int asf_seek_video_stream(uint32_t track, int64_t* pts);

        int asf_seek_audio_stream(int64_t msec, uint32_t track, int64_t* pi8_pts);

        /* seek to the closest (key frame) packet specified by milliseconds position for the specified track */
        int asf_seek_to_msec(int64_t msec, uint32_t track, int64_t* pts);
    ...
    void  asf_parse_WMPicture(uint8_t* WMPicture, uint32_t size, uint32_t* dataoff);
       ...
       // asf_obj_extended_content_description_t* asf_get_extended_content_description();
        ...
        guid_type_t asf_get_sub_format(uint8_t *data, int size);
...
private:
        ...
        guid_type_t asf_guid_get_sub_format_type(const asf_guid_t *guid);
        ...
        int asf_parse_data();
        int asf_parse_index();
        int asf_do_parse_index(int64_t next_index_position);
