					...
					unsigned long flags)
{
	...
	while (remain) {
		...
		if (err) {
			dev_warn(kctx->kbdev->dev, "%s: mmu_get_bottom_pgd failure\n",__func__);
			if (recover_required) {
		...
		mmu_mode->set_num_valid_entries(pgd_page, num_of_valid_entries + count);
	...
	mutex_unlock(&kctx->mmu.mmu_lock);
	mmu_flush_invalidate_insert_pages(kctx->kbdev, &kctx->mmu, start_vpfn, nr);
	return 0;

fail_unlock:
	mutex_unlock(&kctx->mmu.mmu_lock);
	mmu_flush_invalidate_insert_pages(kctx->kbdev, &kctx->mmu, start_vpfn, nr);
	kbase_mmu_free_pgds_list(kctx->kbdev, &kctx->mmu, &free_pgds_list);
	return err;
}
