...
#include <mali_kbase_clock_debugfs.h>
#endif
...
MODULE_PARM_DESC(enable_null_mali, "Enable null mali device driver");
#endif

/* MStar: module parameter to show miu_length of dram */
#ifdef MSTAR
ulong mtk_miu0_length = 0;
module_param(mtk_miu0_length, ulong, S_IRUGO); /* r--r--r-- */
MODULE_PARM_DESC(mtk_miu0_length, "Miu0 length of dram");
ulong mtk_miu1_length = 0;
module_param(mtk_miu1_length, ulong, S_IRUGO); /* r--r--r-- */
MODULE_PARM_DESC(mtk_miu1_length, "Miu1 length of dram");
ulong mtk_miu2_length = 0;
module_param(mtk_miu2_length, ulong, S_IRUGO); /* r--r--r-- */
MODULE_PARM_DESC(mtk_miu2_length, "Miu2 length of dram");
#endif

#if ENABLE_GPU_MFDEC
int mfdec_used_id = -1;
module_param(mfdec_used_id, int, S_IRUGO); /* r--r--r-- */
MODULE_PARM_DESC(mfdec_used_id, "Occupied slot count of gpu-mfdec");
#endif
...
#if MSTAR_UMM_EXPORT
static int kbase_api_mem_umm_export(struct kbase_context *kctx,
		struct kbase_ioctl_mem_umm_export *export)
{
	int err = 0;

#ifdef CONFIG_DMA_SHARED_BUFFER
	export->out.fd = mem_block_export_dma_buf((phys_addr_t)export->in.base,
											  export->in.size,
											  export->in.is_bus_address);
#else
	export->out.fd = -1;
...
#if ENABLE_GPU_MFDEC
static int kbase_api_mfdec_bind_video_buffer(struct kbase_context *kctx, struct kbase_ioctl_mfdec_info *info)
{
	int err = 0;

	if (info->in.id == 0xFF)
	{
		info->out.frame_id = set_mfdec_bridge(info->in.id, info->in.base, info->in.height, info->in.pitch, info->in.is_tiled_yuv);
		info->in.id = (u8)info->out.frame_id;

		if( info->out.frame_id < 0)
		{
			err = MALI_ERROR_FUNCTION_FAILED;
			printk(KERN_DEBUG "[Mali] GPU-MFDEC cannot find any valid slot!\n");
			return err;
		}
	}

	if (info->in.is_tiled_yuv)
		err = set_mfdec_detile(info->in.id, info->in.video_ybase, info->in.video_uvbase, info->in.video_pitch, info->in.encode_type, info->in.flags);
	else
		err = set_mfdec_decode(info->in.id, info->in.video_ybase, info->in.video_uvbase, info->in.BLDbase, info->in.BLDpitch, info->in.encode_type, info->in.flags);
	return err;
}

static int kbase_api_mfdec_unbind_video_buffer(struct kbase_context *kctx, struct kbase_ioctl_mfdec_info *info)
{
	reset_mfdec_slot(info->in.id);
...
#endif
#if MSTAR_UMM_EXPORT
	case KBASE_IOCTL_MEM_UMM_EXPORT:
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_MEM_UMM_EXPORT,
				kbase_api_mem_umm_export,
				struct kbase_ioctl_mem_umm_export);
		break;
#endif
#if ENABLE_GPU_MFDEC
	case KBASE_IOCTL_MFDEC_BIND_VIDEO_BUFFER:
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_MFDEC_BIND_VIDEO_BUFFER,
				kbase_api_mfdec_bind_video_buffer,
				struct kbase_ioctl_mfdec_info);
		break;
	case KBASE_IOCTL_MFDEC_UNBIND_VIDEO_BUFFER:
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_MFDEC_UNBIND_VIDEO_BUFFER,
				kbase_api_mfdec_unbind_video_buffer,
				struct kbase_ioctl_mfdec_info);
		break;
#endif
	}
...
#if defined(MSTAR) && !defined(CONFIG_OF)
	/* Mstar: use clk_get if CONFIG_OF is not enabled */
