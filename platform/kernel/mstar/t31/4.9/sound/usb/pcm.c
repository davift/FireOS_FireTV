...
static int snd_usb_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	dev_err(&subs->dev->dev, "[PCM_USB] sub:endpoint=%d; fmt:endpoint=%c\n", subs->endpoint, subs->cur_audiofmt->endpoint);
			       ...
			       struct urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		if (oldptr + bytes > runtime->buffer_size * stride) {
			...
			memmove(runtime->dma_area + oldptr, cp, bytes1);
			memmove(runtime->dma_area, cp + bytes1, bytes - bytes1);
		} else {
			memmove(runtime->dma_area + oldptr, cp, bytes);
		}
	}
			...
			int offset, int stride, unsigned int bytes)
{
	...
	if (subs->hwptr_done + bytes > runtime->buffer_size * stride) {
		...
		memmove(urb->transfer_buffer + offset + bytes1,
