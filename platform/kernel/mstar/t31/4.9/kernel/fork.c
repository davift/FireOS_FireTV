...
/* reserve 2MB memory to avoid to spend too much time on allocating thread_info which
 * requires 4 pages (order 2) when free memory drops to low
 */
#define LIST_CACHE_CNT  128
...
static unsigned long *alloc_thread_stack_node(struct task_struct *tsk, int node)
{
...
#ifdef CONFIG_MP_CMA_PATCH_DO_FORK_PAGE_POOL
	unsigned long flags;
	gfp_t fastpath_flags = ((THREADINFO_GFP | __GFP_NOWARN | __GFP_NORETRY) & ~__GFP_RECLAIM);
	if(unlikely(atomic_read(&thread_info_cache_cnt) <= 10))
	{
		pr_info_ratelimited("\033[31m[Emergency] %s doing fork, thread_info_cache_cnt %d is extremely low\033[m\n", current->comm, atomic_read(&thread_info_cache_cnt));
#ifdef CONFIG_MP_DEBUG_TOOL_THREAD_CREATE_MONITOR
	...
	/* try no retry no direct reclaim path first */
	page = alloc_pages_node(node, fastpath_flags, THREAD_SIZE_ORDER);
	...
	/* fall back to default linux implementation */
    if (!page)
#endif
	{
