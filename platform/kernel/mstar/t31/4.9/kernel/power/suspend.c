...
static unsigned long str_start_time = 0;
...
unsigned long str_start_time_get(void)
{
	return str_start_time;
}

EXPORT_SYMBOL(str_start_time_get);

void str_time_check(const char *label, const char *module, const char *function)
{
...
static int suspend_prepare(suspend_state_t state)
{
	...
	pr_err("\033[36m%s(%d): setting cpufreq\033[m\n",
	...
	// delete all boost client to avoid client exist after str resume
	del_all_boost_client(0);
#endif
#endif
...
#if defined(CONFIG_MSTAR_PM) && defined(CONFIG_AMZ_MISC)
