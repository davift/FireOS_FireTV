...
/*
* order 8/4 pool are reserved for full screen FB and flyouts. Cache ammount is fixed.
* order 2/0 pool are for generic use. Cache amount is dymanically maintained according to minfree settings
*/
static const unsigned int orders[] = {8, 4, 2, 0};
#else
				      ...
				      unsigned long flags, bool new)
{
	...
	page = ion_page_pool_alloc(pool,flags, new);
	if (cached && page)
...
#ifdef CONFIG_ION_FB_CACHE
extern int allocated_order_8_cache;
#endif
...
#ifdef CONFIG_ION_FB_CACHE
	/* use ION_FB_CACHE_ORDER for only full screen FB buffer */
	unsigned int max_order;
	unsigned int count = PAGE_ALIGN(size) >> PAGE_SHIFT;

	/*
	 *TODO: user order 8/4 pages for flyouts when flyouts use partial screen buffer
	 *Check: KAMINO-8263 and FTVVIZ-28171
	 */
	if (((count == FB_PAGE_COUNT) || (count == FB_AFBC_PAGE_COUNT)) &&
		(ion_buffer_cached(buffer) == ION_FB_CACHE_CACHED) &&
		(orders[0] == ION_FB_CACHE_ORDER)) {
		max_order = 8;
	}
	else
		max_order = 2;

	//pr_info("ion_debug_alloc: %s: rt:%d size is %lu kB, count is %d, flags is %x, max_order is %d, flags is %x\n", current->comm, rt_task(current), size/1024, count, flags, max_order, flags);
#else
