...
fusionee_get_info(FusionDev * dev, FusionGetFusioneeInfo * get_info)
{
     ...
     strncpy( get_info->exe_file, fusionee->exe_file, PATH_MAX - 1 );
     get_info->exe_file[PATH_MAX - 1] = 0;
     return 0;
}
                      ...
                      const void *extra_data, unsigned int extra_size )
{
     ...
     FUSION_ASSERT( msg_size <= FUSION_MAX_PACKET_SIZE );
     FUSION_ASSERT( extra_size <= FUSION_MAX_PACKET_SIZE );
