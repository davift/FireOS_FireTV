...
extern int adsp_ipi_get_wdt_status(struct notifier_block *this,
...
#ifdef CONFIG_MTK_HIFI4DSP_WDT_RECOVER_SUPPORT
	if (is_from_wdt == 1) {
		pr_info("adsp_ipi : send fail because dsp wdt\n");
...
{
	is_from_wdt = 0;
	pr_info("adsp_ipi : set dsp wdt status false!\n");
}
...
#ifdef CONFIG_IPI_LOW_POWER_GVA
