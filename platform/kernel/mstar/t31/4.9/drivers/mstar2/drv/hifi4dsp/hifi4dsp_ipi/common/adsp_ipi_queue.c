...
int scp_flush_msg_queue(uint32_t opendsp_id)
{
	...
	if (!p_adsp_device) {
		pr_notice("%s(), opendsp_id: could not found adsp_device for %u!!\n",
			__func__, opendsp_id);
		return -EFAULT;
	}
	...
	uint32_t wait_ms)
{
	...
	if (buf == NULL || len > SCP_MSG_BUFFER_SIZE) {
		pr_notice("%s(), buf: %p, len: %u!! return\n",
			__func__, buf, len);
		return -EFAULT;
	}
	...
	if (msg_queue->enable == false) {
		pr_notice("%s(), queue disabled!! return\n", __func__);
		return -1;
	}

#ifdef CONFIG_MTK_HIFI4DSP_WDT_RECOVER_SUPPORT
	if (!hifi4dsp_run_status() || is_from_wdt) {
...
#ifdef CONFIG_MTK_HIFI4DSP_WDT_RECOVER_SUPPORT
int adsp_ipi_get_wdt_status(struct notifier_block *this,
		unsigned long event, void *data)
{
	...
	for (idx = 0; idx < MAX_ADSP_COUNT; idx++) {
		p_adsp_device = &g_adsp_devices[idx];
		if (p_adsp_device == NULL) {
			pr_info("%s(), NULL!! p_adsp_device: %p\n",
					__func__, p_adsp_device);
			return -EFAULT;
		...
		if (!hifi4dsp_run_status()) {
			pr_info("DSP shutdown, close ipi and empty scp queue in adsp_ipi_get_wdt_status!\n");
			msg_queue->idx_w = 0;
			msg_queue->idx_r = 0;
		}
	}
