...
int ehci_hub_control(
...
#if (MP_USB_MSTAR==1)
			ehci_err(ehci, "set port %d run before resume\n", wIndex + 1);
	...
	case GetPortStatus:
		...
		if (!ehci->reset_done[wIndex]) {
			...
			if (temp & PORT_RESUME) {
#if (MP_USB_MSTAR==1)
				ehci_err(ehci, "port %d received remote wakeup\n", wIndex + 1);
#endif
				/* resume signaling for 20 msec */
				ehci->reset_done[wIndex] = jiffies
						+ msecs_to_jiffies(20);
		...
		/* resume completed */
			...
			if (retval != 0) {
				...
				ehci_err(ehci, "port %d resume error %d, cmd:%x, sts_old:%x\n",
						wIndex + 1, retval, temp, temp1);
				temp = ehci_readl(ehci, status_reg);
				ehci_err(ehci, "port %d resume error, sts_new:%x\n",
						wIndex + 1, temp);
...
#if (MP_USB_MSTAR==1)
			ehci_err(ehci, "stop port %d run before suspend\n", wIndex + 1);
			temp1 = ehci_readl(ehci, &ehci->regs->command);
			temp1 &= ~(CMD_RUN | CMD_IAAD);
			ehci_writel(ehci, temp1, &ehci->regs->command);
