...
static u32 whitelist_sess;
static u8 whitelist_uuid[TEE_IOCTL_UUID_LEN];

void setup_whitelist_session(u32 session)
{
	whitelist_sess = session;
	pr_crit("allow invoke session:0x%x under STR\n", session);
}
EXPORT_SYMBOL(setup_whitelist_session);

void setup_whitelist_uuid(u8 *uuid, size_t len)
{
	if (len != TEE_IOCTL_UUID_LEN) {
		pr_crit("%s: invaild size\n", __func__);
		return;
	}
	memcpy(whitelist_uuid, uuid, len);
	pr_crit("allow open session with uuid:%pUb under STR\n", (uuid_be *)uuid);
}
EXPORT_SYMBOL(setup_whitelist_uuid);

static void _str_barrier(void)
{
...
void str_barrier_with_sess(u32 session)
{
	if (session == whitelist_sess)
		return;
...
void str_barrier_with_uuid(u8 *uuid)
{
	if (!memcmp(uuid, whitelist_uuid, sizeof(whitelist_uuid)))
		return;
...
void clear_whitelist_session(u32 session)
{
	if (session == whitelist_sess) {
		whitelist_sess = 0;
		pr_info("Clear STR whitelist session id 0x%x\n", session);
	}
}

void clear_whitelist_uuid(u8 *uuid)
{
	if (!memcmp(uuid, whitelist_uuid, sizeof(whitelist_uuid)))
		memset(whitelist_uuid, 0, sizeof(whitelist_uuid));
}
...
static int __init optee_driver_init(void)
{
	...
	if (!IS_ERR(optee)){
		...
		if (!timeout){
			printk(KERN_ALERT "[OPTEE]Create dir /proc/%s/ramlog_setup error!\n",USER_ROOT_DIR);
			return -ENOMEM;
		}
