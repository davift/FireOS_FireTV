...
#ifdef CONFIG_MP_ZRAM_PERFORMANCE
ZRAM_ATTR_RO(write_time);
ZRAM_ATTR_RO(read_time);
ZRAM_ATTR_RO(num_compression);
ZRAM_ATTR_RO(num_decompression);
ZRAM_ATTR_RO(compression_time);
ZRAM_ATTR_RO(decompression_time);
ZRAM_ATTR_RO(compression_time_wo_collision);
ZRAM_ATTR_RO(decompression_time_wo_collision);
ZRAM_ATTR_RO(num_kswapd);
#ifdef CONFIG_MP_MZCCMDQ_HYBRID_HW
ZRAM_ATTR_RO(num_hybrid_hw);
#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	deprecated_attr_warn("collision");
    for(i = 0; i < 10; i++)
...
#ifdef CONFIG_MP_MZCCMDQ_HW
static ssize_t lenc_cycle_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t ldec_cycle_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
#ifdef CONFIG_MP_MZCCMDQ_HYBRID_HW
		zstrm->tfm->base.is_mzc = zram_test_flag(meta, index, ZRAM_IS_MZC);
#endif
#ifdef CONFIG_MP_ZRAM_PERFORMANCE
...
#if defined(CONFIG_MP_MZCCMDQ_HYBRID_HW)	
	if (orig_algo == MZC)  {
		zram_set_flag(meta, index, ZRAM_IS_MZC);
	} else {
		zram_clear_flag(meta, index, ZRAM_IS_MZC);
	}
#endif
...
#ifdef CONFIG_MP_ZRAM_PERFORMANCE
static DEVICE_ATTR_RO(collision);
static DEVICE_ATTR_RO(all_performance_info);
#ifdef CONFIG_MP_MZCCMDQ_HW
static DEVICE_ATTR_RO(lenc_cycle);
static DEVICE_ATTR_RO(ldec_cycle);
#endif
#endif
...
#ifdef CONFIG_MP_ZRAM_PERFORMANCE
	&dev_attr_write_time.attr,
	&dev_attr_read_time.attr,
	&dev_attr_num_compression.attr,
	&dev_attr_num_decompression.attr,
	&dev_attr_compression_time.attr,
	&dev_attr_compression_time_wo_collision.attr,
	&dev_attr_decompression_time.attr,
	&dev_attr_decompression_time_wo_collision.attr,
	&dev_attr_collision.attr,
#ifdef CONFIG_MP_MZCCMDQ_HW
	&dev_attr_lenc_cycle.attr,
	&dev_attr_ldec_cycle.attr,
#endif
	&dev_attr_all_performance_info.attr,
	&dev_attr_num_kswapd.attr,
#endif
	NULL,
};
