...
static unsigned int is_AT_enable;
...
static int __init loglevel(char *str)
{
	...
	if (get_option(&str, &newlevel)) {
		if (is_AT_enable) {
			/* when at_enable which means set console log level to 0
			 * and need to using console mode */
			console_loglevel = 0;
		} else {
			/* force loglevel to 6 to make last kmsg work */
			console_loglevel = 6;
		}
		return 0;
	}
...
static int __init at_enable(char *str)
{
	if (str != NULL) {
		sscanf(str, "%u", &is_AT_enable);
		printk("Is AT enable = %d \n", is_AT_enable);
	}
	return 0;
}
early_param("at_enable", at_enable);
