				   ...
				   struct pstore_info *psi)
{
	...
	int header_length = 0;
	/* force-print uboot logs to console-ramoops for cold boot case */
	...
	*compressed = false;
	...
	if (prz_ok(prz)) {
		...
		if (is_console) {
			...
			/* change type back to PSTORE_TYPE_CONSOLE */
			*id = 0;
		...
		*buf = kmalloc(size + *ecc_notice_size + uboot_log_size + 1, GFP_KERNEL);
		...
		/* print uboot logs to console-ramoops when console log is not available */
		uboot_log_size = get_uboot_log_size();
		...
		*buf = kmalloc(uboot_log_size + 1, GFP_KERNEL);
		if (*buf == NULL)
			return -ENOMEM;

		persistent_uboot_log(*buf);
		console_done = 1;
	}
#ifdef CONFIG_PSTORE_TVMSG
